...
class RandomForestRegressor:
    def __init__(self,base_estimator,n_estimators=10,min_samples_leaf=5,min_samples_split=20):
    ...
    def fit(self,X,y):
        for t in range(self.n_estimators):
            estimator_t=self.base_estimator(random_state=False,min_samples_split=self.min_samples_split,min_samples_leaf=self.min_samples_leaf)
            estimator_t.fit(X,y)
...
if __name__=='__main__':
    ...
    tinyml_decisiontree_reg=tree.DecisionTreeRegressor(min_samples_split=20, min_samples_leaf=5,random_state=False)
